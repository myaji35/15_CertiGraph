...
test.describe('BMad 모의고사 시스템 테스트', () => {
  ...
  test('091. 모의고사 생성 - 기본 설정', async ({ page }) => {
    ...
    await page.goto(`${FRONTEND_URL}/exams/new`);

    // Wait for page to load and study_set selector to be available
    await page.waitForSelector('#exam_study_set_id', { state: 'visible', timeout: 10000 });

    // Select Study Set (ID: 6 created by seeds_test_data.rb)
    await page.selectOption('#exam_study_set_id', '6');
