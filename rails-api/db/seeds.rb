...
# Create document chunks and embeddings for testing
chunk_texts = [
  "객체지향 프로그래밍은 캡슐화, 상속, 다형성이라는 세 가지 핵심 특징을 가지고 있습니다. 캡슐화는 관련된 데이터와 함수를 하나의 단위로 묶는 것이고, 상속은 클래스 간의 관계를 나타내며, 다형성은 같은 인터페이스로 여러 객체를 다룰 수 있는 기능입니다.",
  "TCP와 UDP는 전송 계층에서 동작하는 프로토콜입니다. TCP는 연결을 먼저 수립한 후 데이터를 전송하는 연결 지향적 프로토콜이며, UDP는 연결 수립 없이 바로 데이터를 전송하는 비연결 지향적 프로토콜입니다. TCP는 신뢰성을 보장하지만 느리고, UDP는 빠르지만 신뢰성을 보장하지 않습니다.",
  "스택은 LIFO(Last In First Out) 방식으로 동작하는 자료구조입니다. 먼저 들어온 데이터가 나중에 나오고, 나중에 들어온 데이터가 먼저 나옵니다. 대표적으로 함수 호출 스택, 괄호 검증, 이진수 변환 등에 사용됩니다."
]

# Create document chunks
chunk_texts.each_with_index do |text, index|
  chunk = material.document_chunks.find_or_create_by!(
    chunk_index: index,
    start_position: index * 500,
    end_position: (index + 1) * 500
  ) do |c|
    c.content = text
    c.token_count = (text.length / 4.0).ceil
    c.has_passage = false
  end

  # Create embedding for chunk
  # Note: In real scenario, this would be generated by OpenAI API
  # For testing, we create a dummy embedding (1536 dimensions)
  embedding_vector = Array.new(1536) { rand(-1.0..1.0) }
  magnitude = Math.sqrt(embedding_vector.sum { |v| v ** 2 })

  Embedding.find_or_create_by!(document_chunk: chunk) do |e|
    e.vector = embedding_vector
    e.magnitude = magnitude
    e.generated_at = Time.current
  end
end

puts "✅ Created #{chunk_texts.length} document chunks with embeddings"

# Link some questions to chunks
Question.limit(3).each_with_index do |q, idx|
  chunk = material.document_chunks.where(chunk_index: idx).first
  if chunk
    ChunkQuestion.find_or_create_by!(question: q, document_chunk: chunk)
  end
end

puts "✅ Linked questions to document chunks"
...
puts "\n📊 Embedding System:"
puts "   - #{DocumentChunk.count} document chunks created"
puts "   - #{Embedding.count} embeddings generated"
puts "   - #{ChunkQuestion.count} chunk-question associations created"
